      ...
      <div className="pb-10">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <Link to="/post-job" className="bg-white hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 rounded-2xl p-5 flex items-center gap-4 transition-all group">
              ...
              <div>
                <h4 className="font-bold text-slate-900 group-hover:text-indigo-700 transition-colors">Post a New Job</h4>
                <p className="text-sm text-slate-500">Create a new job listing</p>
              </div>
            </Link>

            <Link to="/manage-jobs" className="bg-white hover:bg-emerald-50 border border-slate-200 hover:border-emerald-200 rounded-2xl p-5 flex items-center gap-4 transition-all group">
              ...
              <div>
                <h4 className="font-bold text-slate-900 group-hover:text-emerald-700 transition-colors">Manage Jobs</h4>
                <p className="text-sm text-slate-500">View and edit your listings</p>
              </div>
            </Link>

            <Link to="/events" className="bg-white hover:bg-purple-50 border border-slate-200 hover:border-purple-200 rounded-2xl p-5 flex items-center gap-4 transition-all group">
              ...
              <div>
                <h4 className="font-bold text-slate-900 group-hover:text-purple-700 transition-colors">Create Event</h4>
                <p className="text-sm text-slate-500">Host a new platform event</p>
              </div>
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <Link to="/manage-jobs" className="text-sm font-bold text-indigo-600 hover:text-indigo-700 transition-colors">
                View all
              </Link>
            </div>
              ...
              {dashboardData.jobs.length > 0 ? (
                dashboardData.jobs.slice(0, 3).map(job => (
                  <Link to={`/applicants?jobId=${job._id}`} key={job._id} className="block w-full">
                    <div className="flex items-center justify-between p-4 rounded-xl border border-slate-100 hover:border-indigo-200 hover:bg-slate-50 hover:shadow-sm transition-all group">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 group-hover:bg-indigo-100 group-hover:text-indigo-600 transition-colors">
                          <Briefcase size={20} />
                        </div>
                        <div>
                      ...
                      <span className={`px-2.5 py-1 text-xs font-bold rounded-full border ${job.isClosed ? 'bg-slate-50 text-slate-500 border-slate-200' : 'bg-emerald-50 text-emerald-600 border-emerald-200'}`}>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-sm text-slate-500">Latest candidate applications</p>
              </div>
              <Link to="/manage-jobs" className="text-sm font-bold text-indigo-600 hover:text-indigo-700 transition-colors">
                View all
              </Link>
            ...
            <div className="space-y-4">
              {dashboardData.recentApplicants.length > 0 ? (
                dashboardData.recentApplicants.map((app, index) => (
                  <Link to={`/applicants?jobId=${app.job}`} key={index} className="flex items-center justify-between p-4 rounded-xl border border-slate-100 hover:border-indigo-200 hover:bg-slate-50 transition-colors group">
                    <div className="flex items-center gap-4">
                      {app.applicant?.profileImage ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-purple-100 text-purple-700 font-bold flex items-center justify-center shrink-0 uppercase">
                          {app.applicant?.fullName?.charAt(0) || 'C'}
                        </div>
                      )}
                      <div>
                        <h4 className="font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{app.applicant?.fullName || 'Candidate'}</h4>
                        <p className="text-xs text-slate-500 mt-1">{app.jobTitle}</p>
                      </div>
                    </div>
